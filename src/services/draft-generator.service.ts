...
const CONTEXT_DIR = join(CONTENT_DIR, 'mcp-context')
...
function loadContext() {
  return {
    brandVoice: readOptional(join(CONTEXT_DIR, 'brand-voice.md')),
    icp: readOptional(join(CONTEXT_DIR, 'icp.md')),
    proofPoints: readOptional(join(CONTEXT_DIR, 'proof-points.md')),
    ctaPolicy: readOptional(join(CONTEXT_DIR, 'cta-policy.md')),
    claimsGuard: readOptional(join(CONTEXT_DIR, 'claims-guard.md')),
  }
}

function parseCompanyServices(): ServiceProfile[] {
  const file = readFileSync(join(CONTENT_DIR, 'company-services.md'), 'utf-8')
  const sections = file
    .split('\n# ')
    .map((s, i) => (i === 0 ? s : `# ${s}`))
...
현장에서는 여전히 수기 보고, 산발적 소통, 조치 이력 누락이 동시에 발생합니다. 이로 인해 같은 문제가 반복되고, 대응 속도와 판단 정확도 모두 떨어질 수 있습니다. ${context.icp ? '특히 본사-현장 간 운영 연속성이 끊기면 관리 비용이 누적되기 쉽습니다.' : ''}
...
${context.proofPoints ? '이 과정에서 실무자는 반복 행정 업무를 줄이고, 관리자는 현황과 리스크를 더 빠르게 파악해 대응할 수 있습니다. 또한 점검/조치 이력이 축적되어 사후 증빙 체계 강화에도 도움이 됩니다.' : '이 과정에서 실무자는 반복 행정 업무를 줄이고, 관리자는 현황과 리스크를 더 빠르게 파악해 대응할 수 있습니다.'}
...
${context.ctaPolicy ? '작은 파일럿부터 시작해 운영 적합성을 검토해보시기 바랍니다.' : ''}
